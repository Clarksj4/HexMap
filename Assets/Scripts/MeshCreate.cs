...
public static class MeshCreate
{
    ...
    /// </summary>
    public static Mesh HexColumn(Vector3 centre, float height, Vector3 normal, float hexSideLength)
    {
        ...
        Mesh top = HexPlane(topCentre, normal, hexSideLength);
        Mesh bottom = HexPlane(bottomCentre, -normal, hexSideLength);
        Mesh sides = HexColumnSides(topCentre, bottomCentre, normal, hexSideLength);
    ...
    /// </summary>
    public static Mesh HexPlane(Vector3 centre, Vector3 normal, float sideLength)
    {
        // Mesh params
        ...
        // Verts
        verts.Add(centre);
        verts.AddRange(HexPlaneVerts(centre, normal, sideLength));
    ...
    /// </summary>
    public static Vector3[] HexPlaneVerts(Vector3 centre, Vector3 normal, float sideLength)
    {
    ...
    /// </summary>
    private static Mesh HexColumnSides(Vector3 topCentre, Vector3 bottomCentre, Vector3 normal, float sideLength)
    {
        Vector3[] topVerts = HexPlaneVerts(topCentre, normal, sideLength);
        Vector3[] bottomVerts = HexPlaneVerts(bottomCentre, -normal, sideLength);
        ...
        for (int i = 0; i < 6; i++)
        {
            Vector3[] quadPoints = new Vector3[] 
            {
                ...
                bottomVerts[GetBottomHexPlaneVertIndex(i + 1)],
                bottomVerts[GetBottomHexPlaneVertIndex(i)]
            };
            ...
            Mesh quad = Quad(quadPoints, quadNormal);
            quads.Add(quad);
        }
    ...
    /// </summary>
    private static Mesh Quad(Vector3[] points, Vector3 normal)
    {
        // Mesh params
        List<int> tris = new List<int>();
    ...
    /// </summary>
    private static int GetBottomHexPlaneVertIndex(int topHexVertIndex)
    {
